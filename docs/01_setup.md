...
> [!IMPORTANT]
> It's important to use version 1.2 of MACI, as this version's circuit are audited and have zKeys which have undergone a trusted setup.
...
> [!IMPORTANT]
> The following steps focus on The Graph, but you can deploy a subgraph using Alchemy.
...
You can also find configurations for the registration, review and tallying periods.
...
- Set `NEXT_PUBLIC_ROUND_ID` to a unique identifier that will group the applications you want to list (can't be too long).
...
> [!IMPORTANT]
> You need to specify either NEXT_PUBLIC_MACI_START_BLOCK or NEXT_PUBLIC_MACI_SUBGRAPH_URL.

#### EAS

If you are running on a different network than Optimism Sepolia you need to update the contract addresses for EAS. These addresses are used whenever an attestation is created.
