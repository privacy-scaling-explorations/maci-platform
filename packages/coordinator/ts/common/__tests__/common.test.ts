...
describe("common", () => {
  describe("getPublicClient", () => {
    ...
    test("should throw when given an unsupported network", () => {
      expect(() => getPublicClient("Unsupported" as ESupportedNetworks)).toThrow(ErrorCodes.UNSUPPORTED_NETWORK);
    });
  });
  ...
  describe("getBundlerClient", () => {
    test("should throw when the network is not supported", () => {
      expect(() => getBundlerClient("Unsupported" as ESupportedNetworks)).toThrow(ErrorCodes.UNSUPPORTED_NETWORK);
    });
  });
  ...
  describe("genAlchemyRPCUrl", () => {
    ...
    test("should throw when given an unsupported network", () => {
      expect(() => genAlchemyRPCUrl(ESupportedNetworks.GNOSIS_CHAIN)).toThrow(ErrorCodes.UNSUPPORTED_NETWORK);
    });

    test("should throw when ALCHEMY_API_KEY is not set", () => {
      delete process.env.RPC_API_KEY;
      expect(() => genAlchemyRPCUrl(ESupportedNetworks.OPTIMISM_SEPOLIA)).toThrow(ErrorCodes.RPC_API_KEY_NOT_SET);
    });
  });
  ...
  describe("viemChain", () => {
    ...
    test("should throw error for unsupported network", () => {
      expect(() => viemChain("UNSUPPORTED_NETWORK" as ESupportedNetworks)).toThrow(ErrorCodes.UNSUPPORTED_NETWORK);
    });
  });
});
