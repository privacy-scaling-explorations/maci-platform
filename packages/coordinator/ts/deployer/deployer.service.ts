...
export class DeployerService {
  ...
  async deployMaci({ approval, sessionKeyAddress, chain, config }: IDeployMaciArgs): Promise<string> {
    ...
    try {
      ...
      const [qvProcessVk, qvTallyVk, nonQvProcessVk, nonQvTallyVk] = await Promise.all([
      ...
      const isProcessVkSet = await publicClient.readContract({
        ...
        args: [
          ...
          config.VkRegistry.args.messageTreeDepth,
          config.VkRegistry.args.voteOptionTreeDepth,
          MESSAGE_TREE_ARITY ** config.VkRegistry.args.messageBatchDepth,
          EMode.QV,
        ],
      });
      ...
      if (isProcessVkSet && isProcessNonQvVkSet && isTallyVkSet && isTallyNonQvVkSet) {
        ...
        if (!receiptVkRegistry.success) {
          throw new Error(ErrorCodes.FAILED_TO_SET_VERIFYING_KEYS_ON_VK_REGISTRY);
        }
      }
    } catch (error) {
    ...
    if (!receipt.success) {
      throw new Error(ErrorCodes.FAILED_TO_SET_MACI_INSTANCE_ON_GATEKEEPER);
    }
  ...
  async deployPoll({ approval, sessionKeyAddress, chain, config }: IDeployPollArgs): Promise<string> {
    ...
    if (!maciAddress) {
      throw new Error(ErrorCodes.MACI_NOT_DEPLOYED);
    }
    ...
    if (!verifierAddress) {
      throw new Error(ErrorCodes.VERIFIER_NOT_DEPLOYED);
    }
    ...
    if (!vkRegistryAddress) {
      throw new Error(ErrorCodes.VK_REGISTRY_NOT_DEPLOYED);
    }
    ...
    try {
      ...
      throw new Error(ErrorCodes.FAILED_TO_DEPLOY_POLL);
    }
    ...
    await Promise.all([
      ...
      this.storage.register({
        ...
        contract: new BaseContract(extContracts[1], AccQueueQuinaryMaciFactory.abi),
        args: [config.messageTreeSubDepth],
        network: chain,
      }),
    ]);
