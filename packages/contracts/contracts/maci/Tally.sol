...
contract Tally is TallyBase, IPayoutStrategy, Pausable {
  ...
  ) public override isInitialized whenNotPaused afterVotingDeadline onlyOwner {
    if (recipientCount == 0) {
  ...
  function claim(
    ...
    uint256 amount = getAllocatedAmount(params.voiceCreditsPerOption, params.totalSpent, params.tallyResult);
    totalAmount -= amount;
  ...
  function getAllocatedAmount(
    ...
    uint256 alpha = calculateAlpha(totalSpent);
  ...
  /// @param totalSpent The total amount of voice credits spent
  function calculateAlpha(uint256 totalSpent) public view returns (uint256) {
